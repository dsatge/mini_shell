...
/*   Updated: 2025/04/21 18:19:54 by enschnei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (ft_strchr(o_cmd->tab[0], '/'))
	{
		if (access(o_cmd->tab[0], F_OK) != 0)
			...
			return (ft_printf(2, "bash: %s: Permission denied\n",
					o_cmd->tab[0]), free_all(minish, 1), exit(126),
				EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
}

static void	check_and_exec_first_attempt(t_minish *minish, t_env_head *env_head,
		t_o_cmd *o_cmd)
{
		...
		t_env_head *env_head)
{
	...
	i = 0;
	path_cmd = NULL;
	check_and_exec_first_attempt(minish, env_head, o_cmd);
	...
	while (minish->pipex->path[i])
	{
		...
		if (!path_cmd)
			return (free_all(minish, 1), perror("strjoin failed"), exit(1));
		if (access(path_cmd, F_OK | X_OK) == 0 && execve(path_cmd, o_cmd->tab,
				minish->pipex->env) == -1)
			return (free_all(minish, 1), exit(127), perror("exe_cmd:"));
		i++;
	}
